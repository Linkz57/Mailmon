...
## which I have now stolen, modified, and bloated out to listen to Nagios.
			...
			Nagios ) if cat $dPATH/mail.txt | grep "Notification Type: PROBLEM"; then
				...
				do
					case $deadservers in
						...
						ONESERVER|oneserver ) printf "oneserver is down! \n\nIn addition to the normal Hyper-V faffery, you'll have to:\n -Boot up the oneserver VM\n -do some other stuff\n -and more still" > $dPATH/out.txt
						;;

						TWOSERVER|twoserver|TwoServer|RedServer|redserver|Redserver|REDSERVER ) printf "Ach! A server is down! There are no unusual steps involved in starting the backup VM. Only the usual steps like: $usualSteps" > $dPATH/out.txt
						;;

						Blueserver|blueserver|BlueServer|BLUESERVER ) printf "BlueServer is down!\n\nGood news : there are no unusual steps involved in starting the backup VM. Bad news : the fish is delish, but makes a small dish. \n\nHere's the usual backup VM startup procedure, if you want it: $usualSteps" > $dPATH/out.txt
						;;

						NEDSERVER|nedserver|Nedserver|NedServer ) printf "NedServer is down!\n\nGood news : there are no unusual steps involved in starting the backup VM. Bad news : He doesn't like his little bed. \n\nAnyway, if you want the usual VM startup steps, here you go: $usualSteps" > $dPATH/out.txt
						;;

						BedServer|bedserver|BEDSERVER ) printf "BedServer is down!\n\nThe BedServer VM's OS works fine and will boot right up if you follow the usual steps, but some other stuff won't work" > $dPATH/out.txt
						;;

						ThreeServer|threeserver|THREESERVER ) printf "ThreeServer is down!\n\nThis is just off-the-cuff musings, here, I don't know if it'll work." > $dPATH/out.txt
						;;

						FourServer ) printf "An important VM is down!\n\nIt should be fine, but just in case..." > $dPATH/out.txt
						;;

						foo|bar ) printf "foobar is down!\n\n    \n\noh well" > $dPATH/out.txt
						;;
